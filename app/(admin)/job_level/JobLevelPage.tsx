...
type JobLevelPagenigation = {
  count: number;
  next: string | null;
  previous: string | null;
  results: JobLevelItem[] | [];
  status: string;
};

type JobLevelPageProps = {
  initJobLevel: JobLevelPagenigation | null;
  initName: string | "";
  initSlug: string | "";
  initLang: string | "";
};
...
export default function JobLevelPage({
  ...
  const [jobLevelList, setJobLevelList] = useState<JobLevelPagenigation>(
    initJobLevel ?? {
      count: 1,
      next: null,
      previous: null,
  ...
  const handleSave = async () => {
    try {
      const response = await axios.post(
        `${process.env.NEXT_PUBLIC_SERVER_URL}job_level/save/`,
        { ...jobLevel },
  ...
  const handleCheckExists = (lang: string, slug: string, name: string) => {
    if (jobLevelList?.results.length === 0) return null;
    const slugExists = jobLevelList.results.find((item) => item.slug === slug);
  ...
  const handleSearch = async (name: string, slug: string, lang: string) => {
    try {
      const response = await axios.get(
        `${process.env.NEXT_PUBLIC_SERVER_URL}job_level/many_search/`,
        {
          params: { name, slug, lang },
        },
      );
      if (response.data.status === "Success") {
        router.push(`/job_level?name=${name}&slug=${slug}&lang=${lang}`, {
          scroll: false,
        });
        setJobLevelList(response.data);
      } else {
        setJobLevelList({
          count: 1,
          next: null,
  ...
  const handleDeleteSlug = async (id: string) => {
    try {
      const response = await axios.delete(
        `${process.env.NEXT_PUBLIC_SERVER_URL}job_level/slug/?id=${id}`,
        { withCredentials: true },
      );
      if (response.data.status === "Success") {
        await handleSearch(search.name, search.slug, search.lang);
  ...
  const handleDeleteTranslation = async (id: string) => {
    try {
      const response = await axios.delete(
        `${process.env.NEXT_PUBLIC_SERVER_URL}job_level/translation/?id=${id}`,
        { withCredentials: true },
      );
      if (response.data.status === "Success") {
        await handleSearch(search.name, search.slug, search.lang);
  ...
  useEffect(() => {
    if (jobLevel.lang) {
      const time = setTimeout(() => {
        setItemExists(
          handleCheckExists(jobLevel.lang, jobLevel.slug, jobLevel.name),
        );
      }, 500);
    ...
    <>
      <div className="flex flex-col w-3/4 max-lg:w-[95%] max-sm:w-full gap-3 max-lg:gap-2 py-3 max-lg:py-2 items-stretch overflow-auto no-scroll min-h-0">
        <div className="flex items-center justify-between p-3 bg-blue-default sm:rounded-2xl shadow-default">
          <p className="font-bold text-light-blue">Danh sách Cấp bậc</p>
          <button
        ...
        <div className="flex-1 flex flex-col gap-2 p-3 bg-white rounded-2xl shadow-default min-h-0">
          ...
          <div className="flex-1 flex gap-2 max-lg:flex-col min-h-0">
            <div className="flex-1 flex flex-col gap-2 p-2 rounded-xl border-2 border-blue-default shadow-default">
              ...
              <div className="flex-1 flex flex-col gap-2 overflow-auto no-scroll max-lg:max-h-100 min-h-100">
                {jobLevelList.results && jobLevelList.results.length > 0 ? (
                  jobLevelList.results.map((item) => (
              ...
              <div className="flex-1 flex flex-col gap-2 overflow-auto no-scroll max-lg:max-h-100 min-h-100">
                {previewLang && previewLang.length > 0 ? (
                  ...
                  <div className="flex-1 flex flex-col justify-center items-center">
                    <PackageOpen className="h-20 w-20 text-zinc-400" />
                    <p className="font-bold text-zinc-400">
                      Cấp bậc này chưa có bản dịch nào
                    </p>
                  </div>
                )}
              </div>
          ...
          <ChangeNumberPage
            next={jobLevelList.next ?? null}
            onNextPage={async () => {
              return await handleChangePage(jobLevelList.next);
            }}
            previous={jobLevelList.previous ?? null}
            onPreviousPage={async () => {
              return await handleChangePage(jobLevelList.previous);
            }}
          />
        </div>
      </div>
            ...
            <div className="flex gap-3 items-center">
              <ButtonDefault
                ...
                disabled={
                  ...
                  (itemExists !== null && itemExists.id !== jobLevel.id)
                }
